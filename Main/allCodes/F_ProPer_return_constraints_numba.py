...
def calc_Kt(KtM, J, PdD, AEdAO, Z):
    Kt = 0
    for i in range(len(KtM)):
        Kt += KtM[i, 0] * (J**KtM[i, 1]) * (PdD**KtM[i, 2]) * (AEdAO**KtM[i, 3]) * (Z**KtM[i, 4])
    return Kt

def F_ProPer_return_constraints_numba(R, Vs, t, w, etaR, zP, Z, D, PdD, AEdAO, hk, Ta):
    # Water input data
    ...
    if Rn > 2e6:
        ...
        while eRn > 0.0001:
            ...
            while Kt > Kts:
                J += 0.1
                Kt = deltaKt + calc_Kt(KtM, J, PdD, AEdAO, Z)
